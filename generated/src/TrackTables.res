...
let trackAllTables = async () => {
  ...
  let _ = await trackTable(~tableName="EventsSummary")
  let _ = await createSelectPermissions(~tableName="EventsSummary")
  let _ = await trackTable(~tableName="RewardFxdxVault_AddReward")
  let _ = await createSelectPermissions(~tableName="RewardFxdxVault_AddReward")
  let _ = await trackTable(~tableName="RewardFxdxVault_SendReward")
  let _ = await createSelectPermissions(~tableName="RewardFxdxVault_SendReward")
  let _ = await trackTable(~tableName="RewardFxdxVault_TotalReserves")
  let _ = await createSelectPermissions(~tableName="RewardFxdxVault_TotalReserves")
  let _ = await trackTable(~tableName="StakedFxdxVault_Stake")
  let _ = await createSelectPermissions(~tableName="StakedFxdxVault_Stake")
  let _ = await trackTable(~tableName="StakedFxdxVault_TotalReserves")
  let _ = await createSelectPermissions(~tableName="StakedFxdxVault_TotalReserves")
  let _ = await trackTable(~tableName="StakedFxdxVault_Unstake")
  let _ = await createSelectPermissions(~tableName="StakedFxdxVault_Unstake")
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="rewardFxdxVault_AddReward",
    ~mappedEntity="RewardFxdxVault_AddReward",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="rewardFxdxVault_SendReward",
    ~mappedEntity="RewardFxdxVault_SendReward",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="rewardFxdxVault_TotalReserves",
    ~mappedEntity="RewardFxdxVault_TotalReserves",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="stakedFxdxVault_Stake",
    ~mappedEntity="StakedFxdxVault_Stake",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="stakedFxdxVault_TotalReserves",
    ~mappedEntity="StakedFxdxVault_TotalReserves",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="stakedFxdxVault_Unstake",
    ~mappedEntity="StakedFxdxVault_Unstake",
  )
}
